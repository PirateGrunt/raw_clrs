...
## Development Factors
In Exercise 1, we calculated development factors by manipulating data structures using `[`, `mutate`, `paste`, and `gather`

We didn't <span class="blue" style: color="blue">**need**</span> to do that since `ChainLadder` offers the `ata` function. 
...
Note that these objects have class `ata` which has certain attributes.
...
(Thanks to Dan Murphy for the following)
```{r}
