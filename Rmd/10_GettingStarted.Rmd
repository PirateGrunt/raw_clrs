...
* echo: To display the output of a code chunk but not the underlying R code

* eval=FALSE: To display R code without evaluating it, you specify the eval=FALSE chunk option:

* warning: (if FALSE, all warnings will be printed in the console instead of the output document)
* error: by default, the evaluation will not stop even in case of errors!! if we want R to stop on errors, we need to set this option to FALSE

* size:font size for the default LaTeX output

* Background: background color of chunks in LaTeX output 
...
* imagess,jpg   ![](path/to/smallorb.png
